...
class HL7Client {
	...
	function __construct($address = null, $port = null, $timeout = 3) {
                $configFile = (str_replace('\\', '/', dirname(__FILE__)) . '/../comun/recursos/xml/config_his.xml');
                if (file_exists($configFile)) {
                    $xmlCargado = simplexml_load_file($configFile);
                }
                
		$this->address = (string) $xmlCargado->config['ipcliente'];
		$this->port = (string) $xmlCargado->config['portclient'];
